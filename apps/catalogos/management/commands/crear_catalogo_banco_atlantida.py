...
class Command(BaseCommand):
    """
    Comando para crear el catálogo de cuentas de Banco Atlántida.
    """
    help = 'Crea el catálogo de cuentas de Banco Atlántida con cuentas predefinidas'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('\n📊 Creando catálogo de Banco Atlántida...\n'))
        ...
        try:
            with transaction.atomic():
                ...
                # Obtener o crear Banco Atlántida
                banco_atlantida, created = Empresa.objects.get_or_create(
                    nombre='Banco Atlántida',
                    defaults={'sector': sector_bancario}
                )
                
                if created:
                    self.stdout.write(self.style.SUCCESS(f'✓ Empresa creada: {banco_atlantida.nombre}'))
                else:
                    self.stdout.write(self.style.WARNING(f'⚠ Empresa ya existe: {banco_atlantida.nombre}'))
                ...
                if created:
                    self.stdout.write(self.style.SUCCESS(f'✓ Catálogo creado para {banco_atlantida.nombre}'))
                else:
                ...
                # Definir las cuentas del catálogo (misma estructura que Banco Agrícola)
                cuentas = [
                    # ACTIVOS
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'\n✓ Catálogo completado: {cuentas_creadas} cuentas creadas para {banco_atlantida.nombre}'
                    )
                )
